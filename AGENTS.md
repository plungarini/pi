...
- **Even G2** (`even-pi`): Intelligent ecosystem for Even Realities G2 smart glasses
...
│   ├── nesthub-pi/              # Nest Hub display service (port 3004)
│   └── even-pi/                 # Even G2 LLM proxy (port 3005)
```
...
- **Fastify**: Web framework
- **better-sqlite3**: Chat history (SQLite)
- **zod**: Config validation
- **cloudflared**: Secure tunnel for Even app access
- **OpenRouter**: Compaction model (Gemini 2.0 Flash Lite)
- **wttr.in**: Free weather API (no auth)
- Depends on: `logger-pi`, `memory-pi` (optional)
...
6. `nesthub-pi` (port 3004) - Depends on logger and Cloudflare
7. `even-pi` (port 3005) - Depends on logger; optionally memory-pi for context
...
└─────────────┘
       ...
       │
┌──────┴──────┬─────────────┬─────────────┬─────────────┬─────────────┬─────────────┐
│  reddit-pi  │ whatsapp-pi │  memory-pi  │ medical-pi  │ nesthub-pi  │  even-pi    │
...
└─────────────┴─────────────┴─────────────┴─────────────┴─────────────┴─────────────┘
       ...
       (even-pi can search/push memory-pi)───────────────────────────────────┘
```
